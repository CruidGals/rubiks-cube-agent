...
import { cubeMoves, updatedCubeMoves, readMoves } from "./playMoveLogic";
...
export function letterToCubeNotation(letter: string, wide_move: boolean = false) {
    switch (letter) {
        case "r": case "R":
...
export function useCubeLogic() {
    ...
    function prepareMove(event: KeyboardEvent) {
        ...
        if (isRotating.value || playingMoves.value) return null;
    ...
    return { masterGroup, prepareMove, playMove, playMoves };
}
